...
public class MenuGerente extends JFrame {
    ...
    private final PlatoManager platoManager;
    private final MermaManager mermaManager;
    private final EmpleadoManager empleadoManager;
    private final PresupuestoManager presupuestoManager;
    private final ProveedorManager proveedorManager;
    ...
    private JPanel crearPanelMermas() {
        ...
        btnRegistrar.addActionListener(e -> {
            ...
            if (ingredienteSeleccionado != null) {
                if (ingredienteSeleccionado.getStock() < cantidad) {
                    JOptionPane.showMessageDialog(this, "Error: No se puede registrar una pérdida mayor al stock actual (" + ingredienteSeleccionado.getStock() + ").");
                    return;
                }
    ...
    private double obtenerPrecioCosto(String nombreIngrediente) {
        try {
            ...
            JOptionPane.showMessageDialog(null,"Error buscando precio: " + e.getMessage());
        }
        return 0.0;
    }
    private void cargarDatosTablas() {
        ...
        if (modeloTablaMermas != null) {
            ...
            List<Merma> mermasValidas = mermasTodas.stream()
                    .filter(m -> m.getIngrediente() != null && m.getIngrediente().getNombre() != null && !m.getIngrediente().getNombre().isEmpty()).sorted(Comparator.comparing(Merma::getFecha).reversed()).collect(Collectors.toList());
    ...
    private JPanel crearPanelPlatos() {
        ...
        btnAgregar.addActionListener(e -> {
            ...
            List<Ingrediente> ingredientesDisponibles = inventarioManager.cargarIngredientes();
            List<JCheckBox> checkBoxes = new ArrayList<>();
            List<JSpinner> spinners = new ArrayList<>();
            if (ingredientesDisponibles.isEmpty()) {
                panelIngredientes.add(new JLabel("No hay ingredientes registrados en el inventario."));
            } else {
                panelIngredientes.add(new JLabel("Seleccione ingredientes y cantidad por plato:"));
                for (Ingrediente ing : ingredientesDisponibles) {
                    JPanel row = new JPanel(new FlowLayout(FlowLayout.LEFT));
                    JCheckBox chk = new JCheckBox(ing.getNombre() + " (" + ing.getUnidadMedida() + ")");
                    JSpinner spinner = new JSpinner(new SpinnerNumberModel(1.0, 0.1, 1000.0, 0.5));
                    ...
                    row.add(chk);
                    row.add(spinner);
                    panelIngredientes.add(row);
                }
            }
            panelFormulario.add(new JScrollPane(panelIngredientes), BorderLayout.CENTER);
            panelFormulario.setPreferredSize(new Dimension(400, 400));
            int option = JOptionPane.showConfirmDialog(this, panelFormulario, "Agregar Nuevo Plato", JOptionPane.OK_CANCEL_OPTION);
                ...
                try {
                    ...
                    for (int i = 0; i < checkBoxes.size(); i++) {
                        if (checkBoxes.get(i).isSelected()) {
                            String nombreIng = ingredientesDisponibles.get(i).getNombre();
                            double cantidadRequerida = (double) spinners.get(i).getValue();
                            Ingrediente ingReceta = new Ingrediente(null, nombreIng, cantidadRequerida, "N/A", "Receta", null);
                            ingredientesSeleccionados.add(ingReceta);
                        }
                    }
                    platoManager.agregarPlato(nombre, precio, descField.getText(), ingredientesSeleccionados);
                    JOptionPane.showMessageDialog(this, "Plato agregado exitosamente con " + ingredientesSeleccionados.size() + " ingredientes.");
                    cargarDatosTablas();
        ...
        btnAgregar.addActionListener(e -> {
            ...
            if (option == JOptionPane.OK_OPTION) {
                try {
                    ...
                    empleadoManager.agregarEmpleado(nombreField.getText(), (String)rolBox.getSelectedItem(), passField.getText(), sueldo);
                    cargarDatosTablas();
                } catch (Exception ex) {
            ...
            if (row != -1) {
                ...
                if (option == JOptionPane.OK_OPTION) {
                    try {
                        ...
                        empleadoManager.modificarEmpleado(nombre, (String)rolBox.getSelectedItem(), nuevoSueldo);
                        cargarDatosTablas();
                    } catch(Exception ex) {
