...
public class Reporte extends JFrame {
    ...
    private JLabel lblGastosVal; // Nuevo para total de compras
    ...
    private void initUI() {
        ...
        JPanel panelResumen = new JPanel(new GridLayout(1, 4, 10, 0)); // 4 Columnas
        panelResumen.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));

        panelResumen.add(crearTarjeta("Dinero en Caja", Color.decode("#27AE60"))); // Verde
        panelResumen.add(crearTarjeta("Ventas Totales", Color.decode("#2980B9"))); // Azul
        panelResumen.add(crearTarjeta("Gastos (Compras)", Color.decode("#E67E22"))); // Naranja
        panelResumen.add(crearTarjeta("Pérdidas (Mermas)", Color.decode("#C0392B"))); // Rojo
        ...
        tabbedPane.addTab("Mermas Registradas", new JScrollPane(new JTable(modeloMermas)));
    ...
    private void cargarDatos() {
        ...
        for (Ingrediente ing : inventario) {
            if (ing.getStock() < 10.0) {
                modeloAlertas.addRow(new Object[]{
                        ing.getNombre(),
                        ing.getStock() + " " + ing.getUnidadMedida(),
                        "CRÍTICO: STOCK BAJO"
                });
                todoOk = false;
            }
        ...
        if (todoOk) {
            modeloAlertas.addRow(new Object[]{"-", "-", "Inventario Óptimo"});
        }
